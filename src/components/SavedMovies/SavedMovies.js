...
import { getLocalStorageValue, saveSavedMoviesToStorage } from "../../utils/localStorageHandlers";
...
function SavedMovies({currentUserId}) {
  ...
  const handleChangeFilters = (newFilterState) => {
    ...
    if (!localMovies || localMovies.length === 0) {
      ...
      getSavedMovies()
        .then((data) => {
          const userCards = data.filter(({ owner }) => owner && owner === currentUserId)
  ...
  const handleChangeCard = (cardData) => {
    const changedCardIndex = cards.findIndex(card => card._id === cardData._id)
  ...
  return (
    <section className="saved-movies">
      ...
      />
      {isLoading && <Preloader />}
      {!isLoading && <MoviesCardList cardItems={cards} isSavedPage={true} handleChangeCard={handleChangeCard} />}
    </section>
  )
}
